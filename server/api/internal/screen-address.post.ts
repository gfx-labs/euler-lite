...
export default defineEventHandler(async (event) => {
  ...
  if (screeningUri) {
    ...
    try {
      ...
      if (isSuspicious) {
        logger.warn({ ctx: 'screen-address', addressHash: hashIdentifier(address) }, 'flagged, malformed, or ambiguous TRM response — failing closed')
      }
  ...
  if (process.env.OFAC_LIST_URL) {
    ...
    if (sanctioned) {
      logger.warn({ ctx: 'screen-address', addressHash: hashIdentifier(address) }, 'OFAC sanctioned address')
    }
